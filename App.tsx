...
const FeatureLayout: React.FC<{ title: string; onBack: () => void; children: React.ReactNode }> = ({ title, onBack, children }) => (
  <div className="flex flex-col h-full bg-slate-950">
    <header className="flex-shrink-0 flex items-center justify-between px-4 pt-[max(16px,env(safe-area-inset-top))] pb-4 border-b border-slate-800 bg-slate-950/80 backdrop-blur-md sticky top-0 z-10">
      <button onClick={onBack} className="p-2 -ml-2 text-slate-400 btn-active"><Icons.Logo className="h-6 w-6 rotate-180" /></button>
...
export default function App() {
  ...
  if (view === 'dashboard') {
    return (
      <div className="h-screen bg-slate-950 flex flex-col p-6 pt-[max(24px,env(safe-area-inset-top))]">
        <div className="flex items-center gap-3 mb-10">
          <div className="bg-cyan-600 p-2 rounded-xl shadow-lg shadow-cyan-600/20">
            <Icons.Logo className="h-6 w-6 text-white" />
          </div>
          <div>
            <h1 className="text-xl font-black tracking-tight text-white">SigNify Study</h1>
            <p className="text-[10px] uppercase tracking-[0.2em] font-bold text-slate-500">Class 10 Sindh Board</p>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4 flex-1">
          {[
            { id: 'qa', label: 'Ask AI Tutor', icon: <Icons.CourseQA className="h-8 w-8" />, desc: 'Concept Q&A', color: 'bg-cyan-600/10 border-cyan-500/30' },
            { id: 'books', label: 'Books', icon: <Icons.Books className="h-8 w-8" />, desc: 'Lesson Solved', color: 'bg-emerald-600/10 border-emerald-500/30' },
            { id: 'translator', label: 'Translator', icon: <Icons.Translator className="h-8 w-8" />, desc: 'Urdu/Sindhi/Eng', color: 'bg-amber-600/10 border-amber-500/30' },
            ...
            <button
              key={item.id}
              onClick={() => { resetState(); setView(item.id as AppView); }}
        ...
        <footer className="mt-auto pt-6 text-center">
          <p className="text-[10px] text-slate-600 uppercase tracking-widest font-bold">Developed by Sike Ali</p>
        </footer>
      </div>
    );
  }
  ...
  return (
    <div className="h-screen max-w-[500px] mx-auto flex flex-col bg-slate-950 relative">
      <FeatureLayout title={view} onBack={() => setView('dashboard')}>
        <div className="p-4 pt-6" ref={scrollRef}>
          {view === 'translator' && transResult && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl mb-6">
                ...
                <h3 className="text-[10px] font-black uppercase tracking-widest text-cyan-400 mb-2">Context</h3>
                  ...
                  <table className="w-full text-xs text-left">
                    <thead className="bg-slate-800">
                      <tr>
                        ...
                        <th className="p-3 text-slate-400">Mean</th>
                      </tr>
                    </thead>
          ...
          {loading && (
            <div className="flex gap-2 animate-pulse mb-8">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 pb-[max(16px,env(safe-area-inset-bottom))] bg-gradient-to-t from-slate-950 via-slate-950 to-transparent">
        <div className="max-w-[500px] mx-auto flex flex-col gap-3">
          ...
          <form onSubmit={handleSend} className="flex gap-2 items-center bg-slate-900 p-2 rounded-2xl border border-slate-800 shadow-2xl">
            <input
              ...
              placeholder={view === 'translator' ? "Enter text..." : "Ask SigNify..."}
